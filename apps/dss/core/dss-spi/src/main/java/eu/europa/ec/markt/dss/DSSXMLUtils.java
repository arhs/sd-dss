...
public final class DSSXMLUtils {
  ...
  /**
   * Document Object Model (DOM) Level 3 Load and Save Specification See: http://www.w3
   * .org/TR/2004/REC-DOM-Level-3-LS-20040407/
   *
  ...
  /**
   * An ID attribute can only be dereferenced if it is declared in the validation context. This behaviour is caused
   * by the fact that the attribute does not have attached type of
   * information. Another solution is to parse the XML against some DTD or XML schema. This process adds the
   * necessary type of information to each ID attribute.
  ...
  /**
   * If this method finds an attribute with names ID (case-insensitive) then it is returned. If there is more than
   * one ID attributes then the first one is returned.
   *
  ...
  /**
   * This method returns the {@link org.w3c.dom.Document} created based on the {@link eu.europa.ec.markt.dss
   ...
   */
  public static byte[] canonicalize(final String canonicalizationMethod, final byte[] toCanonicalizeBytes) throws
      DSSException {
   ...
   */
  public static Element getElementById(Document currentDom, String elementId, String namespace,
                                       String tagName) throws DSSNullException {
   ...
   *
   * @param namespace a {@code HashMap} containing the additional namespace, with the prefix as key and the namespace
   *                  URI as value
   */
