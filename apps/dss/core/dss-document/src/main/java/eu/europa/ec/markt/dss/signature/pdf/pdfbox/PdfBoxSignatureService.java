...
import eu.europa.ec.markt.dss.signature.pdf.*;
...
class PdfBoxSignatureService implements PDFSignatureService {
  ...
  @Override
  public byte[] digest(final InputStream toSignDocument, final SignatureParameters parameters,
  ...
  public void sign(final InputStream pdfData, final byte[] signatureValue, final OutputStream signedStream,
                   ...
                   final DigestAlgorithm digestAlgorithm, final Map.Entry<String,
      PdfDict>... extraDictionariesToAddBeforeSign) throws DSSException {
    ...
    try {
      ...
      signDocumentAndReturnDigest(parameters, signatureValue, signedFile, fileOutputStream, pdDocument, pdSignature,
          digestAlgorithm);
  ...
  private byte[] signDocumentAndReturnDigest(final SignatureParameters parameters, final byte[] signatureBytes,
                                             final File signedFile, final FileOutputStream fileOutputStream,
                                             final PDDocument pdDocument, final PDSignature pdSignature,
  ...
  private void addExtraDictionaries(final PDDocument doc, final Map.Entry<String,
      PdfDict>[] extraDictionariesToAddBeforeSign) {
  ...
  public static void saveDocumentIncrementally(SignatureParameters parameters, File signedFile,
    ...
    try {
      ...
      // the document needs to have an ID, if not a ID based on the current system time is used,
      // and then the digest of the signed data is different
      if (pdDocument.getDocumentId() == null) {

        final byte[] documentIdBytes = DSSUtils.digest(DigestAlgorithm.MD5, parameters.bLevel().getSigningDate()
            .toString().getBytes());
  ...
  @Override
  public void validateSignatures(CertificatePool validationCertPool, InputStream input,
                                 SignatureValidationCallback callback) throws DSSException {
    // recursive search of signature
    Map<String, Set<PdfSignatureOrDocTimestampInfo>> byteRangeMap = new HashMap<String,
        Set<PdfSignatureOrDocTimestampInfo>>();
    final Set<PdfSignatureOrDocTimestampInfo> signaturesFound = validateSignatures(validationCertPool, byteRangeMap,
        null, input);
   ...
   * @param byteRangeMap
   * @param outerCatalog       the PdfDictionary of the document that enclose the document stored in the input
   *                           InputStream
   * @param input              the Pdf bytes to open as a PDF
   * @return
   * @throws DSSException
   */
  private Set<PdfSignatureOrDocTimestampInfo> validateSignatures(CertificatePool validationCertPool, Map<String,
      Set<PdfSignatureOrDocTimestampInfo>> byteRangeMap,
                                                                 PdfDict outerCatalog,
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("Found {} signatures in PDF dictionary of PDF sized {} bytes", signatureDictionaries.size(),
            buffer.size());
      }
      for (int i = 0; i < signatureDictionaries.size(); i++) {
        ...
        try {
          if (PdfBoxDocTimeStampService.SUB_FILTER_ETSI_RFC3161.getName().equals(subFilter)) {
            signatureInfo = PdfSignatureFactory.createPdfTimestampInfo(validationCertPool, outerCatalog, doc,
                signature, cms, buffer);
          } else {
            signatureInfo = PdfSignatureFactory.createPdfSignatureInfo(validationCertPool, outerCatalog, doc,
                signature, cms, buffer);
          }
        ...
        if (innerSignaturesFound == null) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug("Searching signature in the previous revision of the document, size of revision is {} bytes",
        ...
        // need to mark a signature as included inside another one. It's needed to link timestamp signature with the
        // signatures covered by the timestamp.
  ...
  public static PdfSignatureOrDocTimestampInfo signatureAlreadyInListOrSelf(Set<PdfSignatureOrDocTimestampInfo>
                                                                                signaturesFound,
    ...
    for (Iterator<PdfSignatureOrDocTimestampInfo> iterator = signaturesFound.iterator(); iterator.hasNext(); ) {
      ...
      if (existingSignature.uniqueId() == infoId) {
        if (existingSignature instanceof PdfDocTimestampInfo) {
          ...
          if (existingSignatureDocTimestamp.getTimestampToken().getTimeStampType() == TimestampType
              .SIGNATURE_TIMESTAMP) {
            if (LOG.isDebugEnabled()) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Signature newly found {} {}", pdfSignatureOrDocTimestampInfo.getClass().getSimpleName(),
          pdfSignatureOrDocTimestampInfo.uniqueId());
    }
    return pdfSignatureOrDocTimestampInfo;
  }
}
