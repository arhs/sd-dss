	...
	public static InputStream create(final SignatureImageParameters imageParameters) throws IOException {
		...
		if ((textParameters != null) && StringUtils.isNotEmpty(textParameters.getText())) {
			...
			BufferedImage buffImg = ImageTextWriter.createTextImage(textParameters.getText(), textParameters.getFont(), textParameters.getTextColor(),
					textParameters.getBackgroundColor(), DPI);

			if (imageParameters.getImage() != null) {
				switch (textParameters.getSignerNamePosition()) {
					case LEFT:
						buffImg = ImagesMerger.mergeOnRight(ImageIO.read(imageParameters.getImage()), buffImg, textParameters.getBackgroundColor());
						break;
					case RIGHT:
						buffImg = ImagesMerger.mergeOnRight(buffImg, ImageIO.read(imageParameters.getImage()), textParameters.getBackgroundColor());
						break;
					case TOP:
						buffImg = ImagesMerger.mergeOnTop(ImageIO.read(imageParameters.getImage()), buffImg, textParameters.getBackgroundColor());
						break;
					case BOTTOM:
						buffImg = ImagesMerger.mergeOnTop(buffImg, ImageIO.read(imageParameters.getImage()), textParameters.getBackgroundColor());
						break;
					default:
						break;
				}
