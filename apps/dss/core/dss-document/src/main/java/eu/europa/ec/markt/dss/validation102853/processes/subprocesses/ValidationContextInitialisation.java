 ...
 */
public class ValidationContextInitialisation extends BasicValidationProcess implements RuleConstant, Indication, SubIndication, NodeName, NodeValue, AttributeName, AttributeValue, ExceptionMessage {
	 ...
	 * @param parentNode the parent process {@code XmlNode} to use to include the validation information
	 * @return the {@code Conclusion} which indicates the result of the process
	 */
	public Conclusion run(final ProcessParameters params, final XmlNode parentNode) {

		if (parentNode == null) {
			throw new DSSException(String.format(EXCEPTION_TCOPPNTBI, getClass().getSimpleName(), "parentNode"));
		}
		...
		subProcessXmlNode = parentNode.addChild(VCI);
	...
	private boolean checkSignaturePolicyIdentifier(final Conclusion conclusion) {
		...
		constraint.create(subProcessXmlNode, BBB_VCI_ISPK);
