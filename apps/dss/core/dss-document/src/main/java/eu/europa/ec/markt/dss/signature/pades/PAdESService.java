...
public class PAdESService extends AbstractSignatureService {
  ...
  public byte[] getDataToSign(final DSSDocument toSignDocument, final SignatureParameters parameters) throws
    ...
    final byte[] messageDigest = pdfSignatureService.digest(toSignDocument.openStream(), parameters,
        parameters.getDigestAlgorithm());

    SignerInfoGeneratorBuilder signerInfoGeneratorBuilder = padesCMSSignedDataBuilder.getSignerInfoGeneratorBuilder
        (parameters, messageDigest);

    final CMSSignedDataGenerator generator = padesCMSSignedDataBuilder.createCMSSignedDataGenerator(parameters,
        customContentSigner, signerInfoGeneratorBuilder, null);
  ...
  public DSSDocument signDocument(final DSSDocument toSignDocument, final SignatureParameters parameters,
    ...
    try {
      ...
      pdfSignatureService.sign(toSignDocument.openStream(), encodedData, byteArrayOutputStream, parameters,
          parameters.getDigestAlgorithm());
      ...
      if (signatureLevel != SignatureLevel.PAdES_BASELINE_B && signatureLevel != SignatureLevel.PAdES_BASELINE_T &&
          extension != null) {
  ...
  public DSSDocument signDocument(final DSSDocument toSignDocument, final SignatureParameters parameters) throws
    ...
    if (token == null) {
      throw new DSSNullException(SignatureTokenConnection.class, "", "The connection through the available API to the" +
          " SSCD must be set.");
    }
    final byte[] dataToSign = getDataToSign(toSignDocument, parameters);
    final byte[] signatureValue = token.sign(dataToSign, parameters.getDigestAlgorithm(),
