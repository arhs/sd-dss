 ...
 * A.1 Mimetype
 * The "mimetype" object, when stored in a ZIP, file can be used to support operating systems that rely on some
 * content in
 * specific positions in a file (the so called "magic number" as described in RFC 4288 [11] in order to select the
 * specific
 * application that can load and elaborate the file content. The following restrictions apply to the mimetype to
 * support this
 * feature:
 ...
 * An application can ascertain if this feature is used by checking if the string "mimetype" is found starting at
 * offset 30. In
 * this case it can be assumed that a string representing the container mime type is present starting at offset 38;
 * the length
...
public class ASiCContainerValidator extends SignedDocumentValidator {
  ...
  private void createSubordinatedContainerValidators() {
    ...
    for (final DSSDocument signature : signatures) {
      ...
      if (xadesSigned) {
        ...
        throw new DSSException("The format of the signature is unknown! It is neither XAdES nor CAdES, " +
            "nor timestamp signature!");
      }
  ...
  private static void addEntryElement(final String entryName, final List<DSSDocument> list,
                                      final ZipInputStream asicsInputStream) {
   ...
   * specifications.
   * NOTE 4: according to ODF [6] specifications, inclusion of reference to other META-INF information,
   * such as *signatures*.xml, in manifest.xml is optional. In this way it is
  ...
  public static boolean isTimestamp(String entryName) {
    ...
    final boolean timestamp = entryName.endsWith(".tst") && entryName.startsWith(META_INF_FOLDER) && entryName
        .contains("timestamp");
    return timestamp;
  }
  ...
  public static boolean isXAdES(final String entryName) {
    ...
    final boolean signature = entryName.endsWith(".xml") && entryName.startsWith(META_INF_FOLDER) && entryName
        .contains("signature");
    return signature;
  }
  ...
  public static boolean isCAdES(final String entryName) {
    ...
    final boolean signature = entryName.endsWith(".p7s") && entryName.startsWith(META_INF_FOLDER) && entryName
        .contains("signature");
    return signature;
  }
  ...
  private DSSDocument createEmptySignatureDocument() {
    Document signatureDOM = DSSXMLUtils.createDocument(ASiCNamespaces.ASiC, ASiCService.ASICS_NS);
